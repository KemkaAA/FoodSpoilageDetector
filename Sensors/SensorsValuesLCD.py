    ...
    while True:
        if scd.data_available and chanMQ.voltage > 0 and chanPH.voltage > 0:
          ...
          #SGP40
            ...
            mylcd.lcd_clear()
            mylcd.lcd_display_string("CO2:%d Hum:%0.2f Tem:%0.2f" % (co2,humidity,temperature),1)
            mylcd.lcd_display_string("NH4:%0.2f RG:%0.2f VOC:%0.2f" % (voltageMQ,compensated_raw_gas,voc_index),2)
            time.sleep(1)
            mylcd.lcd_clear()
            mylcd.lcd_display_string("Tem:%0.2f" % (temperature),1)
            mylcd.lcd_display_string("VOC:%0.2f" % (voc_index),2)
