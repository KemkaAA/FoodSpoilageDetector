...
# 2.SCD30
    ...
    csv_file_scd = '/home/kemkaa/scd30/scd30.csv'
    csv_file_ph = '/home/kemkaa/MQ137/pH.csv'
    csv_file_mq = '/home/kemkaa/MQ137/mq137.csv'
    csv_file_sgp = '/home/kemkaa/sgp40/sgp40.csv'
    
    try:
        with open(csv_file_scd, 'a+') as f:
            if os.stat(csv_file_scd).st_size == 0:
                f.write('Date,Time,Temperature (°C),CO2 (ppm),Humidity (%)\r\n')
        with open(csv_file_ph, 'a+') as f:
            if os.stat(csv_file_ph).st_size == 0:
                f.write('Date,Time,pH,Voltage(V),Resistance Voltage(rs)\r\n')
        with open(csv_file_mq, 'a+') as f:
            if os.stat(csv_file_mq).st_size == 0:
                f.write('Date,Time,NH4(ppm),Voltage(V),Resistance Voltage(rs)\r\n')
    ...
    while True:
        if scd.data_available and chanMQ.voltage > 0 and chanPH.voltage > 0:
          ...
          #SGP40
            ...
            print(f"MQ-137: {timestamp[0]},{timestamp[1]},{ppm:.4f} ppm,{voltageMQ} V,{rs:.2f} ohms\n")
            print(f"SGP40: {timestamp[0]},{timestamp[1]},{compensated_raw_gas} ppm,{voc_index}\n")
            
            try:    
                with open(csv_file_scd, 'a') as f:
                    timestamp = time.strftime('%m/%d/%y'), time.strftime('%H:%M:%S') 
                    f.write(f"{timestamp[0]},{timestamp[1]},{temperature},{co2},{humidity:.2f}\r\n")
                with open(csv_file_ph, 'a') as f:
                    timestamp = time.strftime('%m/%d/%y'), time.strftime('%H:%M:%S') 
                    f.write(f"{timestamp[0]},{timestamp[1]},{pH_value},{voltagePH},{raw_adc:.2f}\r\n")
                with open(csv_file_mq, 'a') as f:
                    timestamp = time.strftime('%m/%d/%y'), time.strftime('%H:%M:%S') 
                    f.write(f"{timestamp[0]},{timestamp[1]},{ppm:.4f},{voltageMQ},{rs:.2f}\r\n")
                with open(csv_file_sgp, 'a') as f:
                    timestamp = time.strftime('%m/%d/%y'), time.strftime('%H:%M:%S') 
